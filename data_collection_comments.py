...
CONNECTION_STRING = config('CONNECTION_STRING_COMMENTS')
PROJECT_LIST = []
RESPONSE_LIST = []
...
def repo_issues(repo, page):
    ...
    url = str(GITHUB_API) + "repos/" + repo + "/issues/comments?per_page=100&page=" + str(page) + ""
    ...
    for p in PROJECT_LIST.items():
        ...
        if repo_response.text:
            while True:
                ...
                if len(repo_response_page.content) == 32:
                    print(repo_response_page.text)
                    break
